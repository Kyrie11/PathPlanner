...
Planner::Planner() {
  ...
  // TOPICS TO SUBSCRIBE
  if (Constants::manual) {
    subMap = n.subscribe("/trans_map", 1, &Planner::setMap, this);
    ;
  } else {
    subMap = n.subscribe("/occ_map", 1, &Planner::setMap, this);
  }
  subOrigin = n.subscribe("/pointed_map", 1, &Planner::setOrigin, this);
...
void Planner::setOrigin(const nav_msgs::OccupancyGrid::Ptr& map) {
...
//                                   INITIALIZE START
//###################################################
void Planner::setStart(const nav_msgs::Odometry::ConstPtr& initial) {
  if(initial != NULL) {
  float x = (initial->pose.pose.position.x - originX)/ Constants::cellSize;
  float y = (initial->pose.pose.position.y - originY) / Constants::cellSize;
...
void Planner::setGoal(const geometry_msgs::PoseStamped::ConstPtr& end) {
  ...
  float x = (end->pose.position.x - originX)/ Constants::cellSize;
  float y = (end->pose.position.y - originY)/ Constants::cellSize;
...
void Planner::plan() {
  ...
  if (validStart && validGoal) {
    ...
    x = (start.pose.pose.position.x-originX) / Constants::cellSize;
    y = (start.pose.pose.position.y-originY) / Constants::cellSize;
